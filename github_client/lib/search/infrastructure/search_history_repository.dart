import 'package:github_client/core/infrastructure/sembast_database.dart';
import 'package:sembast/sembast.dart';

class SearchHistoryRepository {
  final SembastDatabase _sembastDatabase;
  final _store = StoreRef<int, String>('seatchHistory');
  static const searchLimit = 10;

  SearchHistoryRepository(this._sembastDatabase);

  Stream<List<String>> watchSearchTerms({String? filter}) {
    return _store
        .query(
          finder: filter != null && filter.isNotEmpty
              ? Finder(
                  filter: Filter.custom(
                    (record) => (record.value as String).startsWith(filter),
                  ),
                )
              : null,
        )
        .onSnapshots(_sembastDatabase.instance)
        .map(
          (records) => records.map((record) => record.value).toList(),
        );
  }

  Future<void> addSearchTerm(String term) => _addSearchTerm(
        term,
        _sembastDatabase.instance,
      );

  Future<void> deleteSearchTerm(String term) => _deleteSearchTerm(
        term,
        _sembastDatabase.instance,
      );

  Future<void> putSearchTermFirst(String term) async {
    await _sembastDatabase.instance.transaction((transaction) async {
      await _deleteSearchTerm(term, transaction);
      await _addSearchTerm(term, transaction);
    });
  }

  Future<void> _addSearchTerm(String term, DatabaseClient dbClient) async {
    final termKey = await _store.findKey(
      _sembastDatabase.instance,
      finder: Finder(
        filter: Filter.custom((record) => (record.value as String) == term),
      ),
    );

    if (termKey != null) {
      await putSearchTermFirst(term);
      return;
    }

    await _store.add(_sembastDatabase.instance, term);
    final count = await _store.count(_sembastDatabase.instance);

    if (count > searchLimit) {
      await _store.delete(
        _sembastDatabase.instance,
        finder: Finder(
          limit: count - searchLimit,
        ),
      );
    }
  }

  Future<void> _deleteSearchTerm(String term, DatabaseClient dbClient) async {
    await _store.delete(
      _sembastDatabase.instance,
      finder: Finder(
        filter: Filter.custom((record) => (record.value as String) == term),
      ),
    );
  }
}
